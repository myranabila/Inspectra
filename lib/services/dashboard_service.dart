...
class DashboardService {
  ...
  static Future<List<dynamic>> getDefectsByEquipment({String period = "all"}) async {
    ...
    final response = await ApiService.getList(
      url: '${ApiConfig.baseUrl}/dashboard/analytics/defects?period=$period',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer $token',
      },
  ...
  static Future<void> submitVisualInspectionReport({
    ...
    try {
      final uri = Uri.parse(
        '${ApiConfig.baseUrl}/dashboard/inspections/$inspectionId/submit-visual-report',
      );
      ...
      if (internalAccessible) {
        ...
        request.fields['internal_recommendation'] =
            internalRecommendation ?? 'Nil';
      }
      ...
      print(
        '[SUCCESS] Visual Inspection Report submitted with PDF, status: pending_review',
      );
    } catch (e) {
  ...
  static Future<Map<String, dynamic>?> getUpcomingInspection() async {
    try {
      ...
      scheduled.sort((a, b) {
        final dateA =
            DateTime.tryParse(a['scheduled_date'] ?? '') ??
            DateTime.now().add(const Duration(days: 365));
        final dateB =
            DateTime.tryParse(b['scheduled_date'] ?? '') ??
            DateTime.now().add(const Duration(days: 365));
